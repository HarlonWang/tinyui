...
export function watch<T = unknown>(key: string): () => T | undefined {
    ...
    send("store.subscribe", { key });
    onEmit(`store:${key}`, (p) => setValue(() => ((p as { value: T | null }).value ?? undefined) as T | undefined));
    return value;
}
